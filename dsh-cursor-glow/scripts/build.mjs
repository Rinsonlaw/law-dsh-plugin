// Builds the generated artifacts from lib/glow.js — the SINGLE source of
// truth for the cursor-glow effect:
//   - lib/client.js         (browser bundle, from src/client.template.js)
//   - preview/preview.html  (offline demo, from src/preview.template.html)
//
// lib/glow.js is pure ESM. To inline it into the two non-module artifacts,
...
const glowCode = readFileSync(join(root, 'lib', 'glow.js'), 'utf8')
  .replace(/^export /gm, '')

const MARKER = '/*__GLOW_CODE__*/'
...
function render(templateRel, outRel) {
  ...
  const out = template.replace(MARKER, glowCode)
